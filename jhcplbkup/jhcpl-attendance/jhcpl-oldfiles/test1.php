...
if ($result->num_rows > 0) {
    ...
    echo "<tr><th>ID</th><th>Unit No</th><th>Name</th><th>Date</th><th>Consultation</th><th>ECG</th><th>Echo</th><th>Medicines</th><th>Lab</th><th>Others</th><th>Concession</th><th>Total</th></tr>";
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>" . $row["id"] . "</td><td>" . $row["unit_no"] . "</td><td>" . $row["name"] . "</td><td>" . $row["date"] . "</td><td>" . $row["consultation"] . "</td><td>" . $row["ecg"] . "</td><td>" . $row["echo"] . "</td><td>" . $row["medicines"] . "</td><td>" . $row["lab"] . "</td><td>" . $row["others"] . "</td><td>" . $row["concession"] . "</td><td>" . $row["total"] . "</td></tr>";
    }
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            echo "<tr><td colspan='3'><b>Total</b></td><td>" . $row["SUM(consultation)"] . "</td><td>" . $row["SUM(ecg)"] . "</td><td>" . $row["SUM(echo)"] . "</td><td>" . $row["SUM(medicines)"] . "</td><td>" . $row["SUM(lab)"] . "</td><td>" . $row["SUM(others)"] . "</td><td>" . $row["SUM(concession)"] . "</td><td>" . $row["SUM(total)"] . "</td></tr>";
        }
    } else {
        echo "No data found";
    }
