...
if (isset($_POST['submit'])) {
  ...
  if ($result->num_rows > 0) {
    ...
    // Fetch user_info for the selected unit_no
    $sql = "SELECT name, unit_no, age, sex, diagnosis FROM user_info WHERE unit_no = '$unit_no'";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
    ...
    <form method='post' action=''>
        ...
        <label for='name'>Unit No:</label><br>
        <input type='text' id='unit_no' name='unit_no' value='$unit_no'><br>
        ...
        <input type='text' id='diagnosis' name='diagnosis' value='$diagnosis'><br>
...
if (isset($_POST['add'])) {
    ...
    if ($conn->query($sql) === TRUE) {
        echo "New record with Unit No: $unit_no and Name: $name created successfully";
    } else {
